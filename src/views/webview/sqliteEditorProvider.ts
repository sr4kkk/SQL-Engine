...
export class SqliteEditorProvider implements vscode.CustomReadonlyEditorProvider {
  public static readonly viewType = "dbViewer.sqliteEditor";

  private stateByDoc = new Map<string, DocState>();
...
webviewPanel.onDidChangeViewState(() => {
  if (webviewPanel.visible) scheduleAutoRefresh();
});

webviewPanel.onDidDispose(() => {
  try { clearInterval(poll); } catch {}
  try { if (autoRefreshTimer) clearTimeout(autoRefreshTimer); } catch {}
    ...
    webviewPanel.webview.onDidReceiveMessage(async (msg) => {
      try {
        ...
        if (msg.type === "setPageSize") {
          ...
          if (![50, 100, 200, 500].includes(n)) return;
          st.limit = n;
          st.offset = 0;
          await refreshData();
          return;
        ...
        if (msg.type === "paginate") {
          ...
          if (dir === "prev") st.offset = Math.max(0, st.offset - st.limit);
          if (dir === "next") st.offset = st.offset + st.limit;

          if (dir === "first") st.offset = 0;
        ...
        if (msg.type === "requestDeleteTable") {
          ...
          const choice = await vscode.window.showWarningMessage(
            `Tem certeza que deseja EXCLUIR a tabela '${table}'? Isso é DROP TABLE e não tem volta.`,
            { modal: true },
            "Excluir"
          );
          if (choice === "Excluir") {
...
if (msg.type === "deleteRow") {
  ...
  private async getRowCount(adapter: DbAdapter, table: string): Promise<number | null> {
    try {
      const safe = `"${table.replace(/"/g, '""')}"`;
